#!/usr/bin/env python3
"""Run frozen Qwen3-VL on 30 complete-action calibration inputs."""
...
def main() -> int:
    ...
    try:
        ...
        if (
            manifest.get("adapter") != "temporal_semantic"
            ...
            or not isinstance(events, list) or len(events) != total
        ):
            raise M3JobError("v1.2 input is not the closed 30/0 calibration envelope")
        verified_files = verify_bound_files(cfg.workspace, events)
        verified_resource_bytes = verify_resource_manifest(cfg.resource_manifest, cfg.resource_manifest_sha256, cfg.resource_root)
        ...
        model = Qwen3VLForConditionalGeneration.from_pretrained(
            cfg.model, local_files_only=True, dtype=torch.bfloat16,
        ...
        for ordinal, event in enumerate(events, start=1):
            ...
            with torch.inference_mode():
                generated = model.generate(**inputs, do_sample=False, max_new_tokens=640, use_cache=True)
            ...
            atomic_json(results / f"{event_id}.json", {
                "schema": "quiethand.m3_v1_2.temporal_semantic_result.v1",
                "event_id": event_id, "adapter": "temporal_semantic",
                "status": result_status, "candidate": candidate,
                "raw_text": raw, "failure_reason": failure,
            })
            status(progress, "running", ordinal, total)
        atomic_json(cfg.output / "audit.json", {
            "schema": "quiethand.m3_v1_2.adapter_job_audit.v1",
            "status": "COMPLETE", "adapter": "temporal_semantic",
            "event_count": total, "observed_count": observed, "invalid_count": invalid,
            "input_sha256": cfg.input_sha256, "model_revision": MODEL_REVISION,
            ...
            "runtime": {"python": platform.python_version(), "torch": torch.__version__, "transformers": __import__("transformers").__version__, "cuda": torch.version.cuda, "gpu": torch.cuda.get_device_name(0)},
