#!/usr/bin/env python3
"""Frozen SAM2 + FoundationPose chain for QuietHand action intervals."""
...
from quiethand.object_identity import resolve_role_entity  # noqa: E402
...
def _semantic_candidates(root: Path, expected_count: int, is_evaluation: bool) -> dict[str, dict[str, object] | None]:
    audit = json.loads((root / "audit.json").read_text(encoding="utf-8"))
    expected_opened = True if is_evaluation else False
    if audit.get("status") != "COMPLETE" or audit.get("adapter") != "temporal_semantic" or audit.get("event_count") != expected_count or audit.get("evaluation_results_opened") is not expected_opened:
        raise M3JobError("semantic dependency does not match the frozen input split")
    candidates = {}
    for path in sorted((root / "events").glob("*.json")):
        ...
        if value.get("status") == "observed":
            ...
            validate_candidate(json.dumps(candidate, sort_keys=True, allow_nan=False))
    ...
    if len(candidates) != expected_count:
        raise M3JobError("semantic dependency coverage does not match the frozen input")
    return candidates
...
def main() -> int:
    ...
    try:
        ...
        is_evaluation = manifest.get("schema") == "quiethand.m3_5.evaluation_perception_input.v1"
        calibration_envelope = (
            manifest.get("schema") == "quiethand.m3_v1_2.perception_input.v1"
            and isinstance(expected_count, int)
            and not isinstance(expected_count, bool)
            and 0 <= expected_count <= 30
        ...
        if manifest.get("adapter") != "segmentation_object_state" or not (calibration_envelope or evaluation_envelope) or not isinstance(events, list) or len(events) != expected_count:
            raise M3JobError("segmentation/object input is not a frozen calibration or authorized evaluation set")
        ...
        semantic = _semantic_candidates(args.semantic_output, expected_count, is_evaluation)
        if is_evaluation:
            identity_audit = json.loads((args.identity_output / "audit.json").read_text(encoding="utf-8"))
            identity_ids = {path.stem for path in (args.identity_output / "events").glob("*.json")}
            expected_ids = {event["event_id"] for event in events}
            if (
                identity_audit.get("status") != "COMPLETE"
                or identity_audit.get("event_count") != expected_count
                or identity_audit.get("evaluation_event_count") != expected_count
                or identity_audit.get("evaluation_results_opened") is not True
                or identity_audit.get("training_performed") is not False
                or identity_ids != expected_ids
            ):
                raise M3JobError("identity dependency does not cover the frozen evaluation input one-to-one")
        ...
        for ordinal, event in enumerate(events, start=1):
            ...
            binding = json.loads((args.identity_output / "events" / f"{event_id}.json").read_text())
            candidate = semantic[event_id]
            boxes = {"tool": None, "target": None}
            if candidate is not None:
                boxes = {role: _box_pixels(candidate["start_sample_boxes_0_1000"][role]) for role in boxes}
            ...
            for role in ("tool", "target"):
                ...
                try:
                    ...
                    if entity is None:
                        object_items[role] = {"status": "abstain", "artifact": None, "failure_reason": "object_identity_unresolved"}
                        counts["object_abstain"] += 1
                        continue
                    mesh_path = (args.workspace / entity["mesh_path"]).resolve()
                    ...
                    object_items[role] = {"status": "observed", "artifact": _artifact(args.output, pose_path, dtype="float32", shape=[15, 4, 4], unit="m_SE3", frame_id="camera"), "failure_reason": None, "entity_id": entity["entity_id"], "mesh_path": entity["mesh_path"], "identity_source": binding["source"]}
            ...
            atomic_json(event_dir / "object_state.json", {"schema": "quiethand.object_state.entity_bound.v1", "event_id": event_id, "adapter": "object_state", "items": object_items})
            status(progress, "running", ordinal, total)
        audit = {
            "schema": "quiethand.m3_v1_2.adapter_job_audit.v1",
            "status": "COMPLETE",
