#!/usr/bin/env python3
"""Post-hoc visual/native QA for learned FoundationPose spoon refinement."""
import json
...
from qa_spoon_rgbd_se3 import rotation_errors_deg
...
def main():
    ...
    source = ROOT / 'artifacts/quiethand/m3_5/entity_region_repair/spoon_rgbd_depth_refinement'
    result = ROOT / 'artifacts/quiethand/m3_5/entity_region_repair/spoon_foundation_refinement'
    mesh = trimesh.load(
        ROOT / 'artifacts/quiethand/m3_5/identity_batch/inputs/cads/cad_200.obj',
        process=False)
    sample = np.asarray(mesh.vertices)[::max(1, len(mesh.vertices) // 900)]
    intrinsic = np.loadtxt(taco / 'Egocentric_Camera_Parameters/(put out, spoon, plate)/20231024_056/egocentric_intrinsic.txt')
    ...
    poses = {
        'rgbd_translation': np.load(source / 'pose.npy'),
        'foundation_refined': np.load(result / 'pose.npy'),
        'native_QA': reference,
    }
    rgbs = decode_rgb(
    ...
    for row, index in enumerate(SELECTED):
        for col, label in enumerate(('source', 'rgbd_translation', 'foundation_refined', 'native_QA')):
            ...
            if label != 'source':
                ...
                draw_cloud(image, cloud, intrinsic, '#ff8a22', 1)
    ...
    for index, (image_array, pose) in enumerate(zip(rgbs, poses['foundation_refined'], strict=True)):
        col, row = index % 5, index // 5
        image = Image.fromarray(image_array).resize((384, 216))
        scaled_intrinsic = intrinsic.copy()
        scaled_intrinsic[0] *= 384 / 640
        scaled_intrinsic[1] *= 216 / 360
        cloud = sample @ pose[:3, :3].T + pose[:3, 3]
        draw_cloud(image, cloud, scaled_intrinsic, '#ff8a22', 1)
    ...
    reference_centers = transform_points(reference, centers)[:, 0]
    metrics = {}
    for label in ('rgbd_translation', 'foundation_refined'):
        values = trajectory_errors(
            transform_points(poses[label], centers)[:, 0], reference_centers)
        position = np.asarray(values['position_error_m'])
        rotation = rotation_errors_deg(poses[label], reference)
        metrics[label] = {
            ...
            'mean_rotation_error_deg': float(rotation.mean()),
            'max_rotation_error_deg': float(rotation.max()),
            'rotation_error_deg': rotation.tolist(),
        }
