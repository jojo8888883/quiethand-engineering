#!/usr/bin/env python3
"""Post-hoc visual/native QA for masked RGB-D spoon SE(3) refinement."""
import json
...
def rotation_errors_deg(predicted, reference):
    relative = predicted[:, :3, :3] @ np.transpose(reference[:, :3, :3], (0, 2, 1))
...
def main():
    ...
    result = ROOT / 'artifacts/quiethand/m3_5/entity_region_repair/spoon_rgbd_se3_refinement'
    mesh = trimesh.load(
    ...
    poses = {
        ...
        'rgbd_se3': np.load(result / 'pose.npy'),
        'native_QA': reference,
    }
    rgbs = decode_rgb(
    ...
    for row, index in enumerate(SELECTED):
        for col, label in enumerate(('source', 'rgbd_translation', 'rgbd_se3', 'native_QA')):
    ...
    for index, (image_array, pose) in enumerate(zip(rgbs, poses['rgbd_se3'], strict=True)):
    ...
    metrics = {}
    for label in ('rgbd_translation', 'rgbd_se3'):
