#!/usr/bin/env python3
"""Post-hoc visual and native-pose QA for mask-aware spoon tracking."""
import json
...
def main():
    ...
    per_frame = ROOT / 'artifacts/quiethand/m3_5/entity_region_repair/spoon_per_frame_registration'
    result = ROOT / 'artifacts/quiethand/m3_5/entity_region_repair/spoon_mask_aware_tracking'
    mesh = trimesh.load(
    ...
    poses = {
        'independent': np.load(per_frame / 'pose.npy'),
        'mask_aware': np.load(result / 'pose.npy'),
        'native_QA': reference,
    }
    rgbs = decode_rgb(
    ...
    for row, index in enumerate(SELECTED):
        for col, label in enumerate(('source', 'independent', 'mask_aware', 'native_QA')):
    ...
    for index, (image_array, pose) in enumerate(zip(rgbs, poses['mask_aware'], strict=True)):
    ...
    metrics = {}
    for label in ('independent', 'mask_aware'):
