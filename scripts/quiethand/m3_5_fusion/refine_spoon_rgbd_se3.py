#!/usr/bin/env python3
"""Refine all frozen spoon poses by masked RGB-D local rigid ICP."""
import argparse
...
from mask_depth_se3_icp import refine_se3_projective_icp
...
def main():
    ...
    result = output / 'spoon_rgbd_se3_refinement'
    result.mkdir(exist_ok=True)
    status(result / 'status.json', 'masked_rgbd_se3_icp', 0, 15)
    started = time.monotonic()
    refined, records = [], []
    for index, (pose, depth, mask) in enumerate(zip(poses, depths, masks, strict=True)):
        value, record = refine_se3_projective_icp(
            np.asarray(mesh.vertices), pose, intrinsic, depth, mask,
        ...
        status(result / 'status.json', 'masked_rgbd_se3_icp', index + 1, 15)
    ...
    atomic_json(result / 'audit.json', dict(
        ...
        method='occlusion-aware masked RGB-D projective local rigid ICP',
        rigid_solver='proper-rotation Kabsch', max_iterations=8,
        trim_fraction=0.70, max_distance_fraction_of_mesh_diagonal=0.20,
        minimum_inliers=100, frame_records=records,
        ...
        post_result_tuning_used=False, model_changed=False, gpu_used=False))
